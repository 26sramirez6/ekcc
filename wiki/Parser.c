...
{
  "$end", "error", "$undefined", "\"(\"", "\")\"", "\"+\"", "\"*\"",
  "\"number\"", "$accept", "input", "expr", YY_NULLPTR
};
#endif
...
static void
yy_symbol_value_print (FILE *yyo, int yytype, YYSTYPE const * const yyvaluep, SExpression **expression, yyscan_t scanner)
{
  FILE *yyoutput = yyo;
  YYUSE (yyoutput);
  YYUSE (expression);
...
static void
yy_symbol_print (FILE *yyo, int yytype, YYSTYPE const * const yyvaluep, SExpression **expression, yyscan_t scanner)
{
  YYFPRINTF (yyo, "%s %s (",
             yytype < YYNTOKENS ? "token" : "nterm", yytname[yytype]);

  yy_symbol_value_print (yyo, yytype, yyvaluep, expression, scanner);
  YYFPRINTF (yyo, ")");
}
...
static void
yy_reduce_print (yytype_int16 *yyssp, YYSTYPE *yyvsp, int yyrule, SExpression **expression, yyscan_t scanner)
{
